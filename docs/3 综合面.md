...
# f时间切片
时间切片的本质是模拟实现 requestIdleCallback, requestIdleCallback 是在浏览器一帧中的最后，有空闲时间才会调用。

1. 浏览器的一帧做的事:
task(宏任务: 主线程JS/setTimeout/..) -- job(队列中全部微任务) -- requestAnimationFrame -- 重排/重绘 -- requestIdleCallback
