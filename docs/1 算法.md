...
## f递归遍历 f递归序
每个结点到达三次: x, x左子树绕一圈回来, x右子树绕一圈回来。
```js
...
## f加强堆
增加一个反向索引表，通过 node 反向查询它在堆中的位置，提供 O(1) 的 contains 以及 O(logN) 的 remove,resign, 非常高效🔥
```js
// T 仅支持非基础类型, 如果有基础类型的需求，就让 node 包一层 Inner, 从而拥有独立的内存地址。
class HeapGreater {
  constructor(c) {
    // 存储结点 <T>
    this.heap = [];
...
# f题目数据量的隐含信息 f数据量
一般来说，能`通过`的指令条数在 10^8~10^9 这个范围。 (用 C/C++ 就是 1～2s，用 Java/脚本语言 就是 2～4s)
比如:
1. 题目给的数据量是 10^3, 那么我用一个 O(N^2) 的算法，(10^3)^2 = 10^6 < 10^8, 这是一定能过的。
2. 题目给的数据量是 10^6, 那么用 O(N^2) 是绝对过不了的，至少要想个 O(N^logN) 的。
3. 如果条件里有 N=10^6, V=10^19, P=10^18, 那么算法一定跟 V,P 没有关系，因为如果有关系一定过不了。

# f前缀树
1）单个字符串中，字符从前到后的加到一棵多叉树上
2）字符放在路上，节点上有专属的数据项（常见的是pass和end值）
3）所有样本都这样添加，如果没有路就新建，如有路就复用
4）沿途节点的pass值增加1，每个字符串结束时来到的节点end值增加
5) 这种结构比哈希表要好，因为哈希表没有查询前缀的功能
6) 前缀树加入一个 words 数组的代价是 O(M), M 是所有`单个字符`的数量
7) 前缀树查询一个 word 的代价是 O(K), K 是 word 的字符的数量
8) 前缀树查询有多少 word 以 pre 为前缀的代价是 O(K), K 是 pre 的字符数量
## 前缀树的结构
设计一种结构。用户可以：
...
# f链表
链表问题，coding 无小事。
无论什么问题，你把链表放到容器里，就没有难度了，笔试的时候尽情用吧!
## 面试时链表解题的方法论:
1) 对于笔试，不用太在乎空间复杂度，有容器就用容器，一切为了时间复杂度低 (为了自己能写出来，搞下一道题)
2) 对于面试，时间复杂度依然放在第一位，但是一定要找到空间最省的方法
## 链表常用的手段
1. 容器 (哈希表、数组)
2. 快慢指针
## 题目一、给定一个链表的头结点head, 请判断该链表是否是回文结构
1. 哈希表方法特别简单 (笔试)
2. 改原链表的方法就要注意边界了 (面试)
查看 projects/demo-algorithm/LinkedList/isPalindromeList.js
## 题目二、重排链表
L0 → L1 → L2 -> … -> Ln-2 → Ln-1 → Ln
请将其重新排列后变为:
* L0 → Ln → L1 → Ln-1 → L2 → Ln-2 → … (到达链表中点)
查看 projects/demo-algorithm/LinkedList/reorderList.js
## 题目三、链表中的 partition
查看 projects/demo-algorithm/LinkedList/partition.js
## 题目四、复制带随机指针的链表
查看 projects/demo-algorithm/LinkedList/copyRandomList.js
