...
2. 限制文件大小 
3. 单文件传递给服务器 
  - 传输 (两种方案: 1.FormData 2.Base64)
  - 前端展示缩略图 (base64)
  - 处理文件名 （前端/后端需要处理文件名，使用 SparkMD5，不同的文件内容，就生成不同的 hash 文件名, 这样即使文件同名，只要内容不同就不会冲突。)
  - 进度管控
4. 多文件上传
...
3. TODO: 上传多文件，展示每个文件的上传进度  00:25:10
...
#### f处理文件名 f检测文件是否存在 f哈希 fhash f文件名
