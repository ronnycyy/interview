...
// 包含了这 node_javascript.h 以后，就可以直接使用 node_javascript.cc 里的函数了，比如 DefineJavaScript 下面就用到了。
...
static void Binding(const FunctionCallbackInfo<Value>& args) {
  ...
  if (mod != nullptr) {
    ...
    exports = Object::New(env->isolate());
    // 执行 process.binding('natives') 返回的结果是 DefineJavaScript 函数的执行结果
...
void LoadEnvironment(Environment* env) {
  ...
  // Node.js 执行 lib/internal/bootstrap_node.js 文件以进行初始化启动。
  Local<String> script_name = FIXED_ONE_BYTE_STRING(env->isolate(),
                                                    "bootstrap_node.js");
  // 执行 bootstrap_node.js 以后得到的 f_value 就是 bootstrap_node.js 文件中实现的函数对象                                                 
  ...
  // 得到 f_value 以后，得把它转换成 V8 能用的 Function 对象。
  ...
  // 以 env->process_object() 为参数执行这个函数。
