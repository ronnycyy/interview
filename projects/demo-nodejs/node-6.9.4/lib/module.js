...
// new 一个 Module 的实例，就是一个用户源码模块本体。
function Module(id, parent) {
  this.id = id;
  // 我们平时写的 module.exports 的 module, 就是 Module 类的实例对象。
  // 比如写 module.exports = foo, 就是重新赋值 module 的 exports 变量。
  this.exports = {};
  this.parent = parent;
...
// 检查要 require 的文件的缓存:
// 1.如果缓存中已存在模块：返回其导出对象。
// 2.如果模块为内置模块，则调用 `NativeModule.require()`, 传入文件名，返回结果。
// 3.否则，为文件创建一个新 Module 实例，并将其保存到缓存。然后让它在返回导出之前加载文件内容对象。
Module._load = function (request, parent, isMain) {
  ...
  if (NativeModule.nonInternalExists(filename)) {
    ...
    // ~/lib/internal/bootstrap_node.js: NativeModule.require 函数
...
// 我们平时在项目中执行的 require 函数其实就是这个 Module 类实例对象的 require 函数。
// require 会调用 Module._load 这个静态函数，并声明 isMain (是否是入口模块) 为 false。
